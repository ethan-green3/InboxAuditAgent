...
func main() {
	...
	mailboxUser2 := mustEnv("MAILBOX_USER2")

	users := []string{mailboxUser, mailboxUser2}
	...
	for _, user := range users {
		rootFolders, err := listRootMailFolders(client, token, user, true)
		if err != nil {
			log.Fatalf("failed to list root folders: %v", err)
		}

		allFolders, err := walkAllFolders(client, token, user, rootFolders)
		if err != nil {
		...
		inboxMessages, err := listFolderMessages(client, token, user, inboxFolder.ID, 7, 100)
		if err != nil {
			log.Fatalf("failed to pull Inbox messages: %v", err)
		}

		sentMessages, err := listFolderMessages(client, token, user, sentFolder.ID, 7, 100)
		if err != nil {
		...
		filteredInbox := filterInboxCandidates(inboxMessages, user)
